...
public class ModBlock {
    ...
    public static final Block ChorusSucculent = registerBlock("chorus_succulent",
            new ChorusSucculentBlock(FabricBlockSettings.create().strength(0.0F, 0.0F).mapColor(MapColor.PURPLE).lightLevel((state) -> {
                return 1 + 2 * state.get(ChorusSucculentBlock.Succulent);
            }).sounds(BlockSoundGroup.FUNGUS).nonOpaque().pistonBehavior(PistonBehavior.DESTROY)));
    ...
    private static Block registerBlock(String name, Block block){
        return Registry.register(Registries.BLOCK, new Identifier(EndsDelight.MOD_ID, name), block);
    }
